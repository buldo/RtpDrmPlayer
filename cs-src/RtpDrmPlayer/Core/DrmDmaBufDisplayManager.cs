...
public class DrmDmaBufDisplayManager : IDisposable
{
    ...
    public bool Initialize(uint w,uint h)
    {
        ...
        for(int card=0; card<10; card++)
        {
            ...
            if(PerformModesetDiscovery(w,h)) break;
        }
    ...
    private unsafe bool PerformModesetDiscovery(uint reqW, uint reqH)
    {
        ...
        try
        {
            ...
            int connCount = (int)res.count_connectors;
            int crtcCount = (int)res.count_crtcs;
            int encCount = (int)res.count_encoders;
            uint[] connIds = new uint[connCount];
            uint[] crtcIds = new uint[crtcCount];
            uint[] encIds = encCount > 0 ? new uint[encCount] : Array.Empty<uint>();
            
            fixed (uint* pConn = connIds, pCrtc = crtcIds, pEnc = encIds)
            {
                res.connector_id_ptr = (ulong)(IntPtr)pConn;
                res.crtc_id_ptr = (ulong)(IntPtr)pCrtc;
                if (encCount > 0) res.encoder_id_ptr = (ulong)(IntPtr)pEnc;
            ...
            for (int ci = 0; ci < connCount; ci++)
            {
                ...
                fixed (DrmConst.drm_mode_modeinfo* pModes = modes)
                fixed (uint* pCEnc = cEncIds)
                {
                    gc.modes_ptr = (ulong)(IntPtr)pModes;
                    if (cEncIds.Length > 0) gc.encoders_ptr = (ulong)(IntPtr)pCEnc;
                    if (LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_MODE_GETCONNECTOR, (IntPtr)(&gc)) != 0) continue;
                }
                ...
                {
                    if (modes[m].hdisplay == reqW && modes[m].vdisplay == reqH)
                    {
                        chosenIndex = m;
                        break;
                    }
                ...
                uint selectedCrtc = 0, selectedEnc = 0;
                foreach (var encId in cEncIds)
                {
                    var ge = new DrmConst.drm_mode_get_encoder { encoder_id = encId };
                    if (LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_MODE_GETENCODER, (IntPtr)(&ge)) != 0) continue;
                    if (ge.possible_crtcs == 0) continue;
                    for (int bi = 0; bi < crtcIds.Length; bi++)
                    {
                        if (((ge.possible_crtcs >> bi) & 1u) != 0)
                        {
                            selectedCrtc = crtcIds[bi];
                            selectedEnc = encId;
                            break;
                        }
                    }
                ...
                if (selectedCrtc == 0 && crtcIds.Length > 0) selectedCrtc = crtcIds[0];
                if (selectedCrtc == 0) continue;

                _connectorId = connIds[ci];
                _crtcId = selectedCrtc;
                _mode = modes[chosenIndex];
                _modesetDone = true;
                
                string modeName;
                fixed (byte* pName = _mode.name)
                {
                    modeName = Marshal.PtrToStringAnsi((IntPtr)pName) ?? string.Empty;
                }
                
                Console.WriteLine($"[DRM] Selected card fd={_drmFd} conn={_connectorId} enc={selectedEnc} crtc={_crtcId} mode={_mode.hdisplay}x{_mode.vdisplay} name='{modeName}'");
                return true;
            }
        }
    ...
    public unsafe bool DisplayFrame(FrameInfo f)
    {
        ...
        {
            if (!ImportDmaBuf(f.DmaFd, out gem)) { Console.WriteLine($"[DRM] import failed fd={f.DmaFd}"); return false; }
        }
        ...
        if (_modesetDone)
        {
            ...
            {
                var fb = new DrmConst.drm_mode_fb_cmd2 { width = f.Width, height = f.Height, pixel_format = f.DrmFormat != 0 ? f.DrmFormat : f.Format };
                fb.handles[0] = gem;
                fb.pitches[0] = f.Pitches != null && f.Pitches.Length > 0 ? f.Pitches[0] : f.Width;
                fb.offsets[0] = 0;
                if (f.PlaneCount > 1 && f.Pitches != null && f.Offsets != null)
                {
                    fb.handles[1] = gem;
                    fb.pitches[1] = f.Pitches[1];
                    fb.offsets[1] = f.Offsets[1];
                }
                if (LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_MODE_ADDFB2, (IntPtr)(&fb)) != 0)
                {
                    Console.WriteLine($"[DRM] AddFB2 failed errno={Marshal.GetLastPInvokeError()}");
                }
                else
                {
                    fbId = fb.fb_id;
                    _gemToFb[gem] = fbId;
                    Console.WriteLine($"[DRM] AddFB2 fb={fbId} for GEM={gem}");
                }
            }
            ...
            if (fbId != 0)
            {
                if (_currentFb == 0)
                {
                    ...
                    fixed (uint* pConn = &_connectorId)
                    {
                        crtc.set_connectors_ptr = (ulong)(IntPtr)pConn;
                        crtc.count_connectors = 1;
                        if (LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_MODE_SETCRTC, (IntPtr)(&crtc)) != 0)
                        {
                            Console.WriteLine($"[DRM] SETCRTC failed errno={Marshal.GetLastPInvokeError()}");
                        }
                        else
                        {
                            _currentFb = fbId;
                            Console.WriteLine("[DRM] SETCRTC ok");
                        }
                    }
                }
                else if (fbId != _currentFb)
                {
                    var flip = new DrmConst.drm_mode_crtc_page_flip { crtc_id = _crtcId, fb_id = fbId };
    ...
    private bool ImportDmaBuf(int dmaFd, out uint gemHandle)
    {
        gemHandle = 0;
        if (_drmFd < 0) return false;
        var prime = new DrmConst.drm_prime_handle { fd = dmaFd };
        unsafe { if (LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_PRIME_FD_TO_HANDLE, (IntPtr)(&prime)) != 0) { Console.WriteLine($"[DRM] PRIME_FD_TO_HANDLE errno={Marshal.GetLastPInvokeError()}"); return false; } }
    ...
    private void CloseGem(uint handle)
    {
        ...
        unsafe { LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_GEM_CLOSE, (IntPtr)(&close)); }
    }

    public void Dispose()
    {
        if (_currentFb != 0) { uint id = _currentFb; unsafe { LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_MODE_RMFB, (IntPtr)(&id)); } }
        foreach (var kv in _gemToFb) { uint id = kv.Value; unsafe { LibC.ioctl(_drmFd, DrmConst.DRM_IOCTL_MODE_RMFB, (IntPtr)(&id)); } }
        _gemToFb.Clear();
        foreach (var kv in _dmaToGem) { CloseGem(kv.Value); }
        _dmaToGem.Clear();
        if (_drmFd >= 0) { LibC.close(_drmFd); _drmFd = -1; }
    }
}
