...
public class FrameProcessor
{
    ...
    public bool ProcessDecoded(int idx, ulong used, uint format=0, int planeCount=1, uint[]? pitches=null, uint[]? offsets=null){ if(!_w.HasValue()||!_h.HasValue()){} if(idx<0||idx>=_out.Count) return false; var info=_out[idx]; if(!Validate(idx,info.Mapped,info.Fd)) return false; _decoded++; if(!_zeroCopyInit[idx]){ _setup?.Invoke(idx); _zeroCopyInit[idx]=true; }
